...
def _profile_enabled(*env_names: str) -> bool:
    return any(
...
class TransducerASRModelState(DefaultModelState):
    profile_env_names = ("TRANSDUCER_ASR_PROFILE",)
    profile_label = "Transducer ASR"
    decode_metric_name = "transducer_decode_ms"
    ...
    def __init__(
        ...
        self.prompt_ids_by_req_id: dict[str, int] = {}
    ...
    def remove_request(self, req_id: str) -> None:
        ...
        self.prompt_ids_by_req_id.pop(req_id, None)

    def add_request(self, req_index: int, new_req_data: NewRequestData) -> None:
        super().add_request(req_index, new_req_data)
        if not self.uses_prompt_ids:
            return
        sampling_params = new_req_data.sampling_params
        extra_args = getattr(sampling_params, "extra_args", None) or {}
        prompt_id = extra_args.get("transducer_asr_prompt_id")
        if prompt_id is not None:
            self.prompt_ids_by_req_id[new_req_data.req_id] = int(prompt_id)
    ...
    def get_mm_embeddings(
        ...
        if mm_kwargs:
            profile = _profile_enabled(*self.profile_env_names)
            if profile:
    ...
    def _decode_encoder_outputs(self) -> None:
        ...
        if len(self.encoder_outputs) != len(self.encoder_output_req_ids):
            raise ValueError(
                f"{self.profile_label} encoder output count does not match "
        ...
        if profile:
            _sync_if_cuda(self.device)
            started_at = time.perf_counter()
        if self.uses_prompt_ids:
            hf_config = self.model_config.hf_config
            default_prompt_id = int(hf_config.prompt_id_for_language(None))
            prompt_ids = torch.tensor(
                [
                    self.prompt_ids_by_req_id.get(req_id, default_prompt_id)
                    for req_id in self.encoder_output_req_ids
                ],
                dtype=torch.long,
                device=self.device,
            )
        ...
        if profile:
            ...
            logger.info(
                "%s profile encoder_ms=%.2f %s=%.2f chunks=%d output_tokens=%d",
                self.profile_label,
                self._last_encoder_ms,
                self.decode_metric_name,
                decode_ms,
