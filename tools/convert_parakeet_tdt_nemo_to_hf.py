...
"""Convert NVIDIA Parakeet TDT NeMo checkpoints to vLLM/HF layout.

The public ``nvidia/parakeet-tdt-0.6b-v2`` artifact is a ``.nemo`` archive,
while vLLM's Parakeet TDT implementation loads Hugging Face-style
``config.json`` + ``model.safetensors`` directories. This script converts the
NeMo archive into that layout.
"""
...
def build_parakeet_tdt_config(nemo_config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    token_vocab_size = _as_int(
        _pick(nemo_config, ("joint.num_classes", "decoder.vocab_size"), 8192), 8192
    )
    vocab_size = token_vocab_size + 1
    blank_token_id = vocab_size - 1
    durations = _pick(
        nemo_config,
        ("model_defaults.tdt_durations", "tdt_durations"),
        DEFAULT_DURATIONS,
    )
    if not isinstance(durations, list) or not all(
        isinstance(item, int) for item in durations
    ):
        durations = DEFAULT_DURATIONS

    sample_rate = _as_int(
        _pick(nemo_config, ("sample_rate", "preprocessor.sample_rate"), 16000), 16000
    )
    ...
    encoder_config.update(
        {
            ...
            * _as_int(_get(encoder, "ff_expansion_factor"), 4),
            ...
            "scale_input": bool(_get(encoder, "xscaling", False)),
            "subsampling_conv_channels": _as_int(
                _get(encoder, "subsampling_conv_channels"), 256
            ),
    ...
    return {
        ...
        "max_symbols_per_step": _as_int(
            _get(nemo_config, "decoding.greedy.max_symbols_per_step"), 10
        ),
        "model_type": "parakeet_tdt",
        "num_decoder_layers": _as_int(_get(prednet, "pred_rnn_layers"), 2),
        "pad_token_id": 0,
...
def normalize_nemo_weight_name(name: str) -> str | None:
...
def normalize_state_dict(state_dict: Mapping[str, Any]) -> dict[str, torch.Tensor]:
    ...
    for name, tensor in state_dict.items():
        if not isinstance(tensor, torch.Tensor):
            skipped.append(name)
            continue
        new_name = normalize_nemo_weight_name(name)
        if new_name is None:
            skipped.append(name)
            continue
    ...
    if missing:
        raise ValueError(
            ...
            + ", ".join(missing)
            + ". Check whether this NeMo checkpoint uses a new Parakeet layout."
        )
    ...
    *,
    ...
    _write_json(
        output_dir / "generation_config.json",
        {
            ...
            "suppress_tokens": [
                int(config["vocab_size"]) + idx
                for idx, _ in enumerate(config.get("durations", DEFAULT_DURATIONS))
            ],
        },
    )
    _write_json(
...
def convert_nemo_to_hf(nemo_path: Path, output_dir: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="parakeet-tdt-nemo-") as tmp:
        extract_dir = extract_nemo_archive(nemo_path, Path(tmp))
        nemo_config = load_nemo_config(extract_dir)
        config = build_parakeet_tdt_config(nemo_config)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert a Parakeet TDT .nemo archive to vLLM/HF format."
    )
    parser.add_argument(
        "nemo_path", type=Path, help="Path to parakeet-tdt .nemo archive"
    )
...
def main() -> None:
    ...
    print(f"Wrote converted Parakeet TDT model to {args.output_dir}")
